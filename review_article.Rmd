...
ggplot(cancer) + 
  ...
  scale_fill_distiller("Age-adjusted rate \n (per 100,000)", type = "seq", palette = "RdPu",  direction = 1) + 
...
```{r choropleth-grid-create, message=FALSE, warning = FALSE, fig.cap = "Publicly available choropleth cancer maps, published between 2010 and 2015. Sourced from: @roberts2019communication", out.width='100%', fig.align='center', fig.width=12}
library(cowplot)
...
ggdraw() +
  ...
  draw_plot_label(c("a", "b", "c", "d", "e", "f", "g"), 
    ...
    c(0.98, 0.98, 0.98, 0.74, 0.74, 0.47, 0.47), size = 15) +
...
The [Map of Cancer Mortality Rates in Spain](http://elpais.com/elpais/2014/10/06/media/1412612722_141933.html) can be seen in Figure 2.f. The side by side maps show relative risk of lung cancer for men vs women based on data from 2004 - 2008. The source of the data and statistical methods are unknown. 

### Atlas of Childhood Cancer in Ontario 
Figure 2.g shows the [incidence rate of childhood cancers](https://www.pogo.ca/wp-content/uploads/2015/02/POGO_CC-Atlas-3-Incidence_Feb-2015.pdf) per 100,000, by census division, age 0-14, in Ontario, from 1995 to 2004.

<!-- map is on pg 41 or pdf report-->

Modern mapping methods have the opportunity to incorporate interactive features.
This enables supplementary information to be incorporated into online atlases without cluttering the screen.
Tool tip features and drop down menus allow for user exploration with zooming and panning features to allow flexibility in the display [@HTLWM].
The use of these supports were found in a many online disease map sources.
This directly enables epidemiologists to explore spatial distributions of cancer statistics.
Basic interactivity is often implemented by controls outside of the plot space [@TGA],
this exchanges an image or set of data for another.
Some more advanced interactions include direct interactions with the plot via the use of overlaid tool tip features, very few cancer atlases involve complex selection interactivity.
...
The interactive features of the publically available maps identified by @roberts2019communication allow the exploration of geographic hierarchies, population distribution, statistical uncertainty, demographics and socio-economic indicators.
@TNTEA suggested LM plots as a solution to linking cartography and statistical graphics.


XXX more details on the examples in the next figure
...
ggdraw() +
  ...
  draw_plot_label(c("i", "ii", "iii", "iv", "v"), 
    ...
    c(0.99, 0.99, 0.57, 0.40, 0.15), size = 15) + 
  theme_set(theme_cowplot())
...
## Interaction with atlases

# Cartograms and other spatially warped displays {#ch:alternatives}
...
```{r ggcartograms, fig.cap = "Alternative maps of the United States of America. Each state has been coloured according to the average age-adjusted rate of incidence for lung and bronchus for females and males in the United States 2012-2016. Each stated shape has been distorted according to the population of the state in 2015. The state of California has become much larger due to it's large population density. This draws attention to the densely populated North East region, and detracts from the less populated Mid West.", message=FALSE, warning=FALSE, out.width='100%'}
usa1 <- png::readPNG("figures/ggcont.png")
usa2 <- png::readPNG("figures/ggncont.png")
usa3 <- png::readPNG("figures/ggdorl.png")
#usa4 <- png::readPNG("figures/gghexmap.png")

library(cowplot)
ggdraw() +
  draw_plot(rasterGrob(usa1), 0, .5, 0.5, 0.5) +
  draw_plot(rasterGrob(usa2), 0.5, 0.5, 0.5, 0.5) +
  draw_plot(rasterGrob(usa3), 0.0, 0.0, 0.5, 0.5) +
  #draw_plot(rasterGrob(usa4), 0.5, 0, 0.5, 0.5)+
...
library(cowplot)
ggdraw() +
  draw_plot(rasterGrob(aus1), 0, .5, 0.5, 0.5) +
  draw_plot(rasterGrob(aus2), 0.5, 0.5, 0.5, 0.5) +
  draw_plot(rasterGrob(aus3), 0.0, 0.0, 0.5, 0.5) +
  draw_plot(rasterGrob(aus4), 0.5, 0, 0.5, 0.5)+
  draw_plot_label(c("a", "b", "c", "d"), 
    c(0, .5, 0, 0.5), 
    c(1, 1, .5, .5), size = 15) +
  theme_set(theme_cowplot())
...
<!--
Recent developments in technology have created an opportunity to migrate the presentation of epidological results from atlases in paper map collections to web atlases.
Pairing an explanation with a map or diagram can help to draw attention to a point, group or area of interest.
Maps may form supplementary material to the discussion, in this case static visualisations can help to aid written results. 
Static maps often need to include enough information for the display to be understandable out of the original context.
-->
...
The progress from static print media displays to interactive methods has allowed users to develop maps to ask and answer their own questions.
...
This oppportunity provides an alternative to the traditional approach of a single map for a single use, into a more explorative territory.
...
Developmets and adoptions of this technology has allowed map users more control of the message displayed. Figure \ref{fig:supp-grid-create} presents the control menus for several online cancer atlases.
Users can select combinations of cancer type, gender, and other factors to answers questions relevant to them and their work.
Users can use these controls to change the layer of geography shown, such as continents or countries, states or counties. Thess display can show familiar shapes for users to orient themselves on the map, then zoom in to explore densely populated communities.



<!-- Animations -->
Animations provide a linear, fixed narrative to passive users, communicating a message by capturing and directing attention [@TGA].
...
Animations ensure all users follow the same interpretive path but creators cannnot control the level of engagement.

Animations can be implemented to allow users varying degrees of control. 
A basic form of interactivty with animations is the play, pause and rewind of films, where the film is intended to be watched in one direction but the position in the animation can be controlled.
...
More complex animations may include steps, when user interaction via clicks animates the display between stages.
This approach is implemented by Google Earth 
...
This directs user's attention and gives them tools to plan their own exploration.

<!--
A complex directed animation approach is implemented in steps by Google Earth, in their use of stories. Within these directed steps users have the opportunity to use interactive controls to change the map directions or watch videos. 
One popular implementation communicates Dr. Jane Goodall's life journey using Google Earth displays of significant locations such as her birthplace Bournemouth, England and Tanzania's Gombe National Park. 
-->
...
The following packages were used to create tranformations from the sf [@sf] geographical shapes of the states of America [@spData] and the XXX Australian Statistical Areas (Level 3):
The cartogram [@cartogram] package creates contiguous, non-contiguous and Dorling cartgorams.
