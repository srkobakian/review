---
title: Cancer Applications of Choropleth Maps, and the Potential of Cartograms and
  Alternative Map Displays
...
The white space presents the meaningful empty-space property [@ECGC] [@NAC] but it also distracts the reader from the data, with a low data density [@tufte].
...
## 3.2 Tile Map
<!-- 173 words -->
A tile map provides a tessellated display of consistent shapes.  A similar method to a rectangular cartogram, represents each geographic area using a square. The squares are tessellated to create a grid. Each area is represented by a square of the same dimensions, each tile is usually one unit of measurement. This could be geographic regions such as states or population-based that use a consistent measure of population for each tile. Regions with more than four neighbors introduce some displacement, because only four squares can be adjoining. Like the chorpleth and cartograms, the tile map uses color to represent a value of a statistic for each area. There are online media sources using this method, which include [NPR](https://www.npr.org/2016/10/18/498406765/npr-battleground-map-hillary-clinton-is-winning-and-its-not-close), [FiveThirtyEight](https://projects.fivethirtyeight.com/2016-election-forecast/), [The Wall Street Journal](http://graphics.wsj.com/elections/2016/2016-electoral-college-map-predictions/), [The Washington Post](https://www.washingtonpost.com/graphics/politics/2016-election/50-state-poll/). Tile maps often need to be created manually, requiring more time and care as the number of geographic areas increases.
