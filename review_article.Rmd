...
<!--
Disease maps can rapidly highlight geographical
patterns that would otherwise be hidden in a
table, and are commonly used for hypothesis
generating and as the first step in exploratory
spatial data analysis. @SE
Geography is concerned with the identification and
explanation of spatial structure, pattern, and process,
and with the analysis and explanation of the links
...
<!-- psychology -->
Utilising the familiar state shapes can make maps intuitive to read [@CIBMUK], and allow viewers to visually infer the spatial relationships in the data.
The value of the familiarity of the geography is worth consideration when presenting results of spatial analysis. 
Just as geographers are no longer the only creators of maps, @CPISACA suggests the audience of spatial health data analysis has extended beyond researchers to the public, policymakers and the media.
However, while the areas are recognisable shapes, they are often politically driven boundaries with individual areas being of non-uniform size, different population densities and subject to changes over time. 
The different sizes of administrative areas can attract undue attention to the shades of the unpopulated, large physical areas [@EI]. @GOINO believes there are "severe physiological and psychological constraints on our abilities to assess correlations of spatial distributions visually".
As the display may draw attention from the 'potentially more important results in the more populous communities' [@SE], choropleths can inhibit visual inference when presenting human related statistics. 
...
Almost 100 years of cancer maps have explored the United States and the United Kingdom with increased effectiveness in the presentation of unbiased rates. The increasing development and use of disease maps can be attributed to the availability of location data and associated data points [@GOINO], and geographic information system software [@SE]. Choropleth visualisation suits the data collection methods of census data and cancer mortality rates across regions, and administrative control within regions. 

Mortality rates are now often presented as relative rates of risk across the population, and age adjusted. @HEDP describes Stocks' development of the standardised mortality ratios through the 1930s. The choropleth maps used presented levels of cancer via hatchings on a black and white scale. 
...
@MACM also recognises the intuition that can be derived from choropleths must be 'validated by rigorous statistical analyses.' 
...
Cartograms may be seen as an extension of map tranformations and projections. The favourable distortion is proportional to a value other other than actual earth size area [@NAC].
...
@ACTUC discusses early approaches including John Hunter and Johnathan Young (1968) and Durham's wooden tile method, Skoda and Robertson's (1972) steel ball bearing approach and Tobler's (1973) computer programs.
...
There is no unique solution to the topologic
...
of the various regions are fixed by their popu¬
lations, their shapes and relative locations are
not completely determined.

Other characteristics of the population
may also serve as the base for adjustment; for
example, sex, race, parity, socioeconomic level,
or iminune status.
...
<!--
[-@NAC] observation units stay in more or less their correct location and maintain correct shape, but at least some units are separated by gaps from their neighbors.
...
-There is evidence, too, that angular shapes are more easily seeu than ones with smooth outlines. Shapes that contain the qualities of simplicity and reg,ularity are more readily perceived than others that do not possess these qualities.
...
-The cartographer can oiily efficiently communicate with this kind of map if he has preserved particular elements of the original outline and incorporated them in the new shape.
-
@NISCC 
-->
...
first is the nature or inherent properties of the device itself
second is the ease and accuracy with which map users are able to extract information
from it
