...
```{r supp-grid-create, fig.cap = "Interactive controls of displays in public facing choropleth cancer maps. From left to right and top to bottom: [a. Globocan 2018: Cancer Today](http://globocan.iarc.fr/Pages/Map.aspx)(Controls for indicator, sex, age groups, continents, and cancer types) , [b. Bowel Cancer Australia Atlas](http://www.bowelcanceratlas.org/) (Controls for indicator, age, year, and geographical areas), [c. Global Cancer Map](http://globalcancermap.com/) (Controls for indicator, and cancer type), [d. United States Cancer Statistics: Data Visualizations](https://gis.cdc.gov/Cancer/USCS/DataViz.html) (Controls for demographics, trends, geography, indicators, risk factors, sex, cancer type, and year)."}
i3 <- png::readPNG('cancer_map_images/interactivity/3.interactivity.png')
i15 <- png::readPNG('cancer_map_images/interactivity/15.interactivity.png')
i4 <- png::readPNG('cancer_map_images/interactivity/4.interactivity.png')
i5 <- png::readPNG('cancer_map_images/interactivity/5.interactivity.png')
i7 <- png::readPNG('cancer_map_images/interactivity/7.interactivity.png')
...
supp_grid <- gridExtra::grid.arrange(
  grid::rasterGrob(i3), grid::rasterGrob(i15),
  grid::rasterGrob(i5), grid::rasterGrob(i7), grid::rasterGrob(i4), layout_matrix = lay)

ggsave(filename = "figures/supp_grid.png", plot = supp_grid, device = "png", width = 6, height = 5, dpi = 300)
```
...
| Bivariate display* | N | Y | Y | Y | Y |
* A bivariate display refers to a map that allows two variables to be plotted. One variable via the size of the area, and another by filling the area with a colour.
