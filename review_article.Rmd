...
<!--
@GOINO major geographic storage device; map reading and map interpretation have been the basic methods of retrieval and analysis.
Geography’s interests are in spatial distributions and associations, in areal differentiation
and in spatial processes. All of these interests demand information that is geographically
ordered. Geographically ordered data are those data which have their locational
coordinates preserved.

Certain limitations to the standard graphic techniques have also become apparent, however.
They are slow, and do not permit instant retrieval of information. They are untrustworthy when interpretations of spatial association are needed; there are both severe physiological and psychological constraints on our abilities to assess correlations of
spatial distributions visually.

These opportunities will be lost if locational coding is not inserted into the data referencing system as such banks are built, however.

There are six potential benefits to geographic research from computer-connected data banks in which the data have proper locational referents. These come under the general headings of mapping, partitioning and aggregation, merging of data systems, transformations of geographic spaces, descriprive statistics, and intermediate analysis.
...
The key difference is the desirable augmentation of the size, shape or distance of geographical areas [@ACTUC].
...
When visualising population statistics @ACTUC considers this equitable representation design 'more socially just, giving due attention to all members of the population and reducing the visual impact of large areas with small populations [@DMAHP].
...
@CTTMB believes there is likely 'to manifest a growing demand for urban thematic maps'.
...
-->
@NAC Their intention is to allow the map space to highlight the distribution of the variable. However a reader may have to know the difference between initial geography and new layout given by a cartogram, to be able to recognise the significant changes.

@CBATCC conflicting tasks: adjusting region sizes and retainingregion shapes. in context of agricultural applications

Distortion of shape on the contiguous cartogram presents an additional hurdle to visual recognition and this hurdle is not only eliminated on the noncontiguous cartogram but is replaced by the meaningful empty-space property. @NAC


Cartograms keep spatial relationships of neighbours intact by preserving borders when adjusting sizes [-@NAC]. Tobler's Conformal mapping means to preserve angles
...
@CBATCC Through the spatial transformation of map regions relative to the data, the cartogram
prominently emphasizes data distribution instead of territorial size.
-better visualized on a map more representative of population.
-A continuous area cartogram is one in which the topology, or connectivity, of the map regions
is retained while areas are resized.
-explains Tobler’s Pseudo-Cartogram Method, Dorling’s Cellular Automaton Method, Radial Expansion Method of Selvin et al., Rubber Sheet Method of Dougenik et al., and Gusein-
Zade and Tikunov’s Line Integral Method.
- compare to their Constraint-Based Method (Kocmoud and House)
