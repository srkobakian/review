...
Choropleth maps colour each geographic unit to allow map users to measure the value of the statistic [@EI]. Map users contrast the colours in neighbouring areas to understand the spatial distribution. Map creators must choose an attractive and effective colour scheme, but is a difficult task [@CB]. The ColorBrewer system supports map creators in the decision making process, but this does not consider perceptual colour specifications. When communicating information using colour, a map creator should use a scheme that has a linear color gradient, with perceptually uniform color spaces to convey equal steps in data align with equal steps in the colour scheme [@PUCS]. 

Brewer [@CB] also suggests the use and colors of borders and backgrounds can change the appearance of the colors representing the value of the statistics.
Incorporating borders may allow a reference point to orient users as they compare colours of neighbouring areas. 
