...
```{r ggtilemap, fig.cap = "A Tile map and geofaceted map of the Unites States of America. Each state has been coloured according to the average age-adjusted rate of incidence for lung and bronchus for females and males in the United States 2012-2016. Each state has been represented by a square, each square is the same size. The neighbouring states are easily contrasted, however the north east regions has been displaced from their geographic location. The sparsity of the population in the Mid West is highlight by the light yellow colour, the age adjusted rate in Kentucky is the darkest and its neighbours are similar.", message=FALSE, warning=FALSE, fig.width=12, fig.height=6}
  
# Import the images
t1 <- png::readPNG('figures/ggtilegram.png')
t2 <- png::readPNG('figures/ggfacet.png')

ggdraw() +
  draw_plot(rasterGrob(t1), 0, 0, 1, 1) +
  draw_plot(rasterGrob(t2), 1, 0, 1, 1) +
...
Interaction covers manipulation of the graph directly, via the graph controls or command-line [@RPIG].
...
These controls may also change the geographies shown, such as continents or countries, states or counties. These display can show familiar shapes for users to orient themselves on the map, then zoom in to explore densely populated communities.
Interactions with online atlases can also change the amount of detail shown regarding the display.
...
Linked displays allow for direct manipulation, @RPIG uses Swayne and Klinke's definition of high level interaction that occurs via data selection or brushing tools that highlight subsets of graphical elements such as polygons or points. Queries form a low level interaction, returning values or information used in the graphical display without referring to the original data source.
...
- this is one reference that maybe you have missed: https://alsino.io/static/papers/BeyondChoropleths_AlsinoSkowronnek.pdf
