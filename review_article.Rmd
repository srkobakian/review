...
A simple tilegram presents the areas in a tessellated grid display, where geographic neighbours are found next to each other, with some necessary displacement employed for regions with more than four neighbours.
...
Geofaceting utilises a grid display, the arrangement of tiles creates a grid that mimics the geographic topology of the set of areas @IGF. 
Like tilegrams the arrangement of the areas can be reused for other spatial distribution visulisations.
Unlike tilegrams geofaceting replicates visualisations for each geographic subset of the data.
The amount of information able to be communicated has increased from one value per region to one visualisation, this is a more flexible display, especially as it provides a simple way to communicate time series data.
