...
Choropleths are univariate displays, they do not inherently support multi-dimensionality. This makes it difficult to pair demographic, environmental and other factors with spatial distributions.
@MMST present linked micromap plots as a solution to the issue of multi-dimensionality when visually linking geographic and statistical data.
This design is widely used in online atlas displays, it groups areas of similar values for one variable, and uses the additional columns to provide displays that contrast the states in each group by other variables.
The display juxtaposes of choropleth maps and statistical plots.
Showing one map per group of the key separating variable, in a row with each additional statistical plot.
...
A tilegram provides a tessellated display of consistent shapes.
...
A simple tilegram may present the states in a tessellated grid display, where geographic neighbours are found next to each other, with some necessary displacement employed for regions with more than four neighbours.
...
When there is a relationship between the cancer type being mapped and geographical features, a choropleth map would pair this information visually. However these displays are limited to uni-variate displays, usually presented by filling each area with a colour.
These maps are best used when the geography is of importance to the data in the display.
They favor geographically large regions and small regions are often represented by inset displays that zoom in on a geographic area containing many small regions, this is seen in Figure \ref{fig:choropleth-grid-create} c of the state of Queensland, where the capital city Brisbane is highlighted by the inset map. 
...
The change in the map can also provide opportunities for bi-variate displays, using the change in area for one variable, and colour for another.
...
<!--
Recent developments in technology have created an opportunity to migrate the presentation of epidemiological results from atlases in paper map collections to web atlases.
...
Direct manipulation of web displays allow continuous view of the map, which change due to the physical actions of users. The affects of these actions are rapid, incremental, and reversible @DMIV.

Developments and adoptions of this technology has allowed map users more control of the message displayed. Figure \ref{fig:interacting} presents the control menus for several online cancer atlases.
...
Users can use these controls to change the layer of geography shown, such as continents or countries, states or counties. These display can show familiar shapes for users to orient themselves on the map, then zoom in to explore densely populated communities.
