...
```{r choropleth-grid-create, message=FALSE, fig.cap = "Public facing choropleth cancer maps. From right to left and top to bottom: [a. Globocan 2012: Estimated  Cancer Incidence, Mortality and Prevalence](http://globocan.iarc.fr/Pages/Map.aspx)  (Incidence rates per 100,000 for all cancers, male and female),  [b. Bowel Cancer Australia Atlas](http://www.bowelcanceratlas.org/), [c. Atlas of Cancer in QLD - Relative incidence ration, Lung cancer in males](https://cancerqld.org.au/research/queensland-cancer-statistics/queensland-cancer-atlas/), [d. CINA+ Online Cancer in North America](http://www.cancer-rates.info/naaccr/) (Incidence rates of all invasive cancers per 100,000, male and female), [e. Atlas of Childhood Cancer in Ontario](http://www.pogo.ca/research-data/pogo-atlas/) (Incidence rate of childhood cancer per 100,000, men and women), [f. An Atlas of Cancer in South Australia](https://www.cancersa.org.au/assets/images/pdfs/An%20Atlas%20of%20Cancer%20in%20South%20Australia%20-%20Full%20Report.pdf) (Incidence rate per 100,00, lung cancer in females), [g. The Environment and Health Atlas of England and Wales](http://www.envhealthatlas.co.uk/eha/Breast/) (Lung cancer in females), [h. Map of Cancer Mortality Rates in Spain](http://elpais.com/elpais/2014/10/06/media/1412612722_141933.html) (Relative risk, Lung cancer, men vs women), [i. United States Cancer Statistics: An Interactive Cancer Statistics Website](https://nccd.cdc.gov/DCPC_INCA/) (Incidence rate per 100,000, all cancer types, men and women"}
...
plot1 <- png::readPNG('cancer_map_images/choropleths/3.choropleth.png')
plot2 <- png::readPNG('cancer_map_images/choropleths/15.choropleth.png')
plot3 <- png::readPNG('cancer_map_images/choropleths/25.choropleth.png')
plot4 <- png::readPNG('cancer_map_images/choropleths/20.choropleth1.png')
plot5 <- png::readPNG('cancer_map_images/choropleths/26.choropleth.png')
plot6 <- png::readPNG('cancer_map_images/choropleths/14.choropleth2.png')
plot7 <- png::readPNG('cancer_map_images/choropleths/21.choropleth.png')
plot8 <- png::readPNG('cancer_map_images/choropleths/23.choropleth.png')
plot9 <- png::readPNG('cancer_map_images/choropleths/7.choropleth.png')
plot10 <- png::readPNG('cancer_map_images/choropleths/30.choropleth.png')
plot11 <- png::readPNG('cancer_map_images/choropleths/29.choropleth.png')
plot12 <- png::readPNG('cancer_map_images/choropleths/1.png')
plot13 <- png::readPNG('cancer_map_images/choropleths/24.choropleth.png')
plot14 <- png::readPNG('cancer_map_images/choropleths/6.choropleth.png')
plot15 <- png::readPNG('cancer_map_images/choropleths/27.choropleth.png')
plot16 <- png::readPNG('cancer_map_images/choropleths/8.choropleth.png')
...
While cancer maps are powerful visualisations that summarise complex statistical analyses, the statistics represented in these maps cannot tell the entire story. Supplementary graphs and plots are often included that add more depth and more information to the map. Maps can also utilise bar charts, tables and scatter-plots as useful additional displays. 
These displays can provide alternative views of the cancer statistics, as well as the supporting statistical outputs such as error, confidence intervals, distributions, sample/population sizes, standard deviation, as well as others.. 
When presenting cancer maps, @MACM believes the intuition derived from maps must be 'validated by rigorous statistical analyses.' These supplementary statistics aim to enable this validation. @CPISACA suggests additional materials such as 'good tables, graphs, and explanatory text' support understanding and inference derived from maps, ensuring the message communicated will be consistent across a range of viewers. 

<!-- insert figure with 3 examples of supplementary graphs-->

**Interactivity**   

The interactivity of modern maps enables supplementary information to be incorporated without cluttering the screen, such as in a tool tip feature. The use of these supports were found in a variety of online map sources. These interactive features provide additional information to help users understand and interpret the spatial distribution presented, as well as validate, explain or explore the presented statistics and their relationships to each other and/or their underlying spatial distribution. The interactive features of the publically avaialble maps identified by @roberts2019communication were....... 
...
The groundwork for the simplistic approach had been laid in the mid 1930's be Raisz. 
...
Square grids have been used by FiveThirtyEight, Bloomberg Business, The Guardian, The Washington Post, The New York Times and NPR. Where each area is represented by a square of the same dimensions.
An example is provided in \ref{fig:ggtilegram}
...
|  | Choropleth | Contiguous Cartogram | Non-contiguous Cartogram | Dorling | Tilegrams |
|:---------------------------------------------------:|------------|----------------------|--------------------------|---------|-----------|
| Feature preservation: |  |  |  |  |  |
| Preserves shapes | Y | N | Y | N | N |
| Preserves neighbours | Y | Y | Y | S | S |
| Uniform use of shape | N | N | N | Y | Y |
| Statistical transformation of the base map: |  |  |  |  |  |
| Used for bi-variate maps eg. population & statistic | N | Y | Y | Y | Y |
