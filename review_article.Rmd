...
(insert figure of both types of interactivity)
...
(insert figures showing different interactivity)
...
## Micromaps
XXX Something needs to be said about micromaps https://cran.r-project.org/web/packages/micromapST/vignettes/JSSmicromapSTv63i03.pdf
...
library(cowplot)
ggdraw() +
  draw_plot(rasterGrob(aus1), 0, .5, 0.5, 0.45) +
  draw_plot(rasterGrob(aus2), 0.5, 0.5, 0.5, 0.45) +
  draw_plot(rasterGrob(aus3), 0.0, 0.0, 0.5, 0.45) +
  draw_plot(rasterGrob(aus4), 0.5, 0, 0.5, 0.45) +
  draw_plot(aus_legend, 0, 0, 1, 0.1) +
  draw_plot_label(c("a", "b", "c", "d"), 
    c(0, .5, 0, 0.5), 
    c(1, 1, .5, .5), size = 20)
```
...
A tilegram provides a tesselated display of consistent shapes.
Each tile is usally one unit of measurement, this could be geographic regions such as states, or population based where one tile is used for a consistent measure of population.
<!-- cite mosiac cartograms --> 
<!--XXX cite geogrid here?-->
A simple tilegram may present the states in a tesselated grid display, where geographic neighbours are found next to each other, with some neccessary displacement employed for regions with more than four neighbours.
These tiles may be labelled or coloured to represent a value.
A data visualisation can be geographically faceted, so that a smaller visualisation of equal size is created for each geographic subset of the data. This requires grid displays that are often created once by hand and reused.
...
When there is a relationship between the cancer type being mapped and geographical features, a choropleth map would pair this information visually. However these displays are limited to univariate displays, usually presented by filling each area with a colour.
These maps are best used when the geography is of importance to the data in the display.
They favor geographically large regions and small regions are often represented by inset displays that zoom in on a geographic area containing many small regions, this is seen in Figure \ref{fig:choroleth-grid-create} c of the state of Queensland, where the capital city Brisbane is highlighted by the inset map. 
...
Cartograms with the population variable used to distortion the geographic map can be used as density equalising maps, that reduce the visual prominence of low-population areas [@VSSDCUC]. 
...
The unusual shapes are difficult to compare and can prevent comparison of the statistic. Cartograms also favour extreme statistics in the positive direction. Small extreme values are often lost due to the resulting small sized areas the do not draw user's attention.
The change in the map can also provide opportunities for bivariate displays, using the change in area for one variable, and colour for another.
...
Dorlings provide bivariate displays as they use the size of the shape for one variable, and colour for another.
...
XXX Define interactivity, and also animation
...
- the arcGIS standard https://pro.arcgis.com/en/pro-app/help/mapping/animation/overview-of-animation.htm
- very general infovis reference that refers to Buja, Cook, Swayne distinguishing between direct manipulation and interactive graphics https://hal.inria.fr/tel-01096366/file/VA2_PERIN_CHARLES_17112014.pdf
- explanation of inetractivity and direct manipulation: https://books.google.com.au/books?id=A6TMqxkW-EoC&pg=PA6&lpg=PA6&dq=direct+manipulation+%22swayne%22+%22klinke%22&source=bl&ots=tXYKP8JbbV&sig=ACfU3U3KhN-_YNt_UV5SmThsl4D2gY9A4g&hl=en&sa=X&ved=2ahUKEwj6u9vgqtzkAhVKeysKHQ6iByUQ6AEwA3oECAgQAQ#v=onepage&q=direct%20manipulation%20%22swayne%22%20%22klinke%22&f=false
- might be useful to look at https://towardsdatascience.com/a-complete-guide-to-an-interactive-geographical-map-using-python-f4c5197e23e0
- a classic http://www.cs.umd.edu/hcil/trs/2001-08/2001-08.html
- this is one reference that maybe yoou have missed: https://alsino.io/static/papers/BeyondChoropleths_AlsinoSkowronnek.pdf
